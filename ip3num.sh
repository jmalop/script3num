...
ports=$(cat results | awk '{print $1}' | sed 's/\/tcp/,/g' | tr -d "\n" | sed '$ s/.$//' | sed 's/,1SF.*")//g' | sed 's/------PORT//g' | sed 's/,Nma//g' | sed 's/------Some//g' | sed 's/PORT//g' | sed 's/,Servic//g'  | sed 's/------//g')
echo $ports
...
do
        ...
        then
                ...
                gobuster dir -u http://$1:$portww/ -r -k -t 100 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -x txt,php,html,asp --wildcard --timeout 20s >> temp1
